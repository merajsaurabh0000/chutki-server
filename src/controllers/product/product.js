  ...
  try {
    ...
    if (authHeader && authHeader.startsWith("Bearer ")) {
      ...
      if (decoded.role === "Customer") {
        const customer = await Customer.findById(decoded.userId).select("phone");
        if (customer && customer.phone === 9999999999) {
          return true;
        }
      }
    }
...
export const getNearbyCatalog = async ({ location, categoryId, isDemo }) => {
  ...
  if (!eligibleBranches.length) {
    if (isDemo && branches.length > 0) {
      eligibleBranches = [{ branch: branches[0], distance: 0 }];
    } else {
...
export const getProductsByCategoryId = async (req, reply) => {
  ...
  try {
    if (location) {
      ...
      const { products } = await getNearbyCatalog({ location, categoryId, isDemo });
