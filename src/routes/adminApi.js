...
const sanitizeFilename = filename => {
  ...
  const nameWithoutExt = String(filename || "").split(".").slice(0, -1).join(".") || "image";
  const safeName = nameWithoutExt.replace(/[^a-zA-Z0-9-]/g, "_").trim() || "image";
...
const uploadBannerFile = async (file, bannerId) => {
  ...
  let buffer = file._buf || file.file?.uploadBuffer || file.file?._readableState?.buffer?.head?.data;
  
  if (!buffer && file.file && !file.file.readableEnded) {
    try {
      buffer = await streamToBuffer(file.file);
    } catch (err) {
      // ignore
    }
  }

  if (!buffer && typeof file.toBuffer === "function") {
    try {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (buffer && buffer.length > 0) {
      upload.end(buffer);
    } else if (file.file && !file.file.readableEnded) {
      file.file.on("error", reject);
      file.file.pipe(upload);
    } else {
      reject(new Error("File stream is empty or has already been consumed."));
    }
  });
};
...
const uploadProductFile = async file => {
  ...
  const buffer = file._buf || file.file?.uploadBuffer;
...
const uploadProductFromUrl = async (url) => {
  try {
...
export const adminApiRoutes = async fastify => {
  ...
  fastify.post("/admin/products/upload-image", { preHandler: [requireAdminToken, requireSuperAdmin] }, async (request, reply) => {
    const productFile = request.body?.image || await request.file();
    ...
    const imageKey = `${result.public_id}.${result.format}`;
    const secureUrl = `https://res.cloudinary.com/${CLOUDINARY_CLOUD_NAME}/image/upload/${encodeCloudinaryKey(imageKey)}`;

    // Save image to the database Media Library
    await Models.UploadedImage.create({
      filename: productFile.filename || "image.jpg",
      url: secureUrl,
  ...
  fastify.get("/admin/images", { preHandler: [requireAdminToken, requireSuperAdmin] }, async (request) => {
    const q = String(request.query?.q || "").trim();
    const filter = q ? { filename: new RegExp(escapeRegex(q), "i") } : {};
    
    const images = await Models.UploadedImage.find(filter).sort({ createdAt: -1 }).lean();
    return {
  ...
  fastify.delete("/admin/images/:id", { preHandler: [requireAdminToken, requireSuperAdmin] }, async (request, reply) => {
    const { id } = request.params;
    const img = await Models.UploadedImage.findById(id);
  ...
  fastify.post("/admin/products/bulk", { preHandler: [requireAdminToken, requireSuperAdmin] }, async (request, reply) => {
    ...
    // Download and upload image URLs to Cloudinary for unique products in batches
    const concurrencyLimit = 5;
    for (let i = 0; i < uniqueDocs.length; i += concurrencyLimit) {
      const batch = uniqueDocs.slice(i, i + concurrencyLimit);
      await Promise.all(
        batch.map(async (doc) => {
          if (doc.image && (doc.image.startsWith("http://") || doc.image.startsWith("https://")) && !doc.image.includes("res.cloudinary.com")) {
            doc.image = await uploadProductFromUrl(doc.image);
          }
        })
      );
    }
  ...
  fastify.get("/admin/kaggle/preview", { preHandler: [requireAdminToken, requireSuperAdmin] }, async (request, reply) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error("Kaggle download response error:", response.status, errorText);
        return reply.code(response.status).send({ message: `Kaggle download failed: Status ${response.status}` });
      }

      console.log("Dataset downloaded. Extracting zip entries...");
      ...
      if (!csvEntry) {
        ...
        console.error("CSV file not found in ZIP entries");
      ...
      console.log(`Parsing CSV file: ${csvEntry.entryName}`);
      ...
      console.error("Kaggle preview failed with error:", error);
      return reply.code(500).send({ message: `Kaggle preview error: ${error.message}` });
    }
  });
  ...
  fastify.post("/admin/banners", { preHandler: [requireAdminToken] }, async (request, reply) => {
    ...
    const banner = await Models.Banner.create({
      ...
      sortOrder,
      isActive,
      imageFilename: sanitizeFilename(bannerFile.filename),
      imageMimeType: bannerFile.mimetype,
    });

    try {
      bannerFile._buf = fileBuffer;
      ...
      console.error("Banner creation upload error:", error);
      await Models.Banner.deleteOne({ _id: banner._id });
      throw error;
    }
  });

  fastify.patch("/admin/banners/:bannerId", { preHandler: [requireAdminToken] }, async (request, reply) => {
  ...
  fastify.put("/admin/banners/:bannerId", { preHandler: [requireAdminToken] }, async (request, reply) => {
    const { bannerId } = request.params;
    ...
    if (bannerFile) {
      ...
      let result;
      try {
        result = await uploadBannerFile(bannerFile, banner._id);
      } catch (error) {
        console.error("Banner update upload error:", error);
        throw error;
      }
      banner.imageKey = `${result.public_id}.${result.format}`;
      banner.imageSize = result.bytes;
      banner.imageFilename = sanitizeFilename(bannerFile.filename);
  ...
  fastify.delete("/admin/banners/:bannerId", { preHandler: [requireAdminToken] }, async (request, reply) => {
    const { bannerId } = request.params;
    const banner = await Models.Banner.findById(bannerId);
