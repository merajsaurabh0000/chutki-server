...
const originConfig = origins.length > 0 ? origins : true;

await app.register(cors, {origin: originConfig, credentials: true});
await app.register(rateLimit, {max: 120, timeWindow: "1 minute"});
await app.register(rawBody, {global: false, encoding: "utf8", runFirst: true});
await app.register(fastifySocketIO, {cors: {origin: originConfig}, transports: ["websocket"]});
...
try { await app.listen({port: Number(PORT), host: "0.0.0.0"}); }
